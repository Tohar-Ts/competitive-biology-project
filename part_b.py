...
def compare_gb_to_up(cds_id, up_id, clean_cds_id, clean_up_id):
    ...
    print("Missing:", up_missing_len, ", Nan values: ",
          up_nan_len, ", from total:", up_total_len)
...
def create_trans_table(up_df):
    ...
    for __, row in up_clean_df.iterrows():
        ...
        trans = trans.replace('TRANSMEM', '').replace(
            ' ', '').replace('..', '-')
        trans = trans.split(';')
...
def plot_cds_trans_gc_percent(cds, cds_with_trans, cds_without_trans):
    ...
    plt.subplot(1, 4, 3)
    plot_hist("Proteins without transmembrane",
...
if __name__ == "__main__":
    ...
    # printing the comparing results
    compare_gb_to_up(cds_id, up_id, clean_cds_id, clean_up_id)

    # Q2
    print("\n------------Question 2------------")
    # creating a table of all the trans parts
    ...
    # printing trans hydro amino stats
    ...
    cds_with_trans, cds_without_trans = group_cds_by_trans(
        cds_df, clean_cds_id, clean_trans_id)
